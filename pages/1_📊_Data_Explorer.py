...
# Add root directory to sys.path to import from src and config
...
m3.metric("Missing Values", summary['missing_total'])
    ...
    with st.expander("🔍 Filter Data"):
        ...
        with col2:
            if "Graduation_Status" in df.columns:
                grad_status = st.multiselect("Graduation Status", [0, 1], default=[0, 1])
            else:
                grad_status = []
...
with tab2:
    ...
    with col1:
        ...
        plot_type = st.radio("Plot Type", ["Histogram", "Boxplot (Outliers)"])
        color_by = st.selectbox("Color By (Optional)", ["None", "Graduation_Status", "Placement_Status"])
